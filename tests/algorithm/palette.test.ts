...
// 8 FigJam manual cases: 4 white theme + 4 black theme
const figmaCases = [
...
describe('buildPalette — FigJam cases: WCAG contrast targets', () => {
  for (const { input, theme } of figmaCases) {
    describe(`${input} / ${theme}`, () => {
      ...
      it('shade 300 has >= 3.1:1 contrast on shade 700', () => {
        expect(contrastRatioHex(palette['300'].hex, palette['700'].hex)).toBeGreaterThanOrEqual(3.1)
      })

      it('shade 600 has >= 3.1:1 contrast on shade 100', () => {
        expect(contrastRatioHex(palette['600'].hex, palette['100'].hex)).toBeGreaterThanOrEqual(3.1)
      })

      it('shade 800 has >= 3.1:1 contrast on shade 600', () => {
        expect(contrastRatioHex(palette['800'].hex, palette['600'].hex)).toBeGreaterThanOrEqual(3.1)
      })

      it('shade 900 has >= 3.1:1 contrast on shade 700', () => {
        expect(contrastRatioHex(palette['900'].hex, palette['700'].hex)).toBeGreaterThanOrEqual(3.1)
      })
    })
  }
})
...
describe('find300', () => {
  it('white theme: result has >= 3.1 contrast against shade700', () => {
    const shade700 = find700(parseHex('#1A6646'), 'white')
    expect(contrastRatioHex(find300(shade700, 'white'), shade700)).toBeGreaterThanOrEqual(3.1)
  })

  it('black theme: result has >= 3.1 contrast against shade700', () => {
    const shade700 = find700(parseHex('#1C734E'), 'black')
    expect(contrastRatioHex(find300(shade700, 'black'), shade700)).toBeGreaterThanOrEqual(3.1)
  })
})

describe('find600', () => {
  it('white theme: result has >= 3.1 contrast against shade100', () => {
    const shade700 = find700(parseHex('#1A6646'), 'white')
    const shade100 = find100(shade700, 'white')
    expect(contrastRatioHex(find600(shade100, 'white'), shade100)).toBeGreaterThanOrEqual(3.1)
  })

  it('black theme: result has >= 3.1 contrast against shade100', () => {
    const shade700 = find700(parseHex('#1C734E'), 'black')
    const shade100 = find100(shade700, 'black')
    expect(contrastRatioHex(find600(shade100, 'black'), shade100)).toBeGreaterThanOrEqual(3.1)
  })
})

describe('find800', () => {
  it('white theme: result has >= 3.1 contrast against shade600', () => {
    const shade700 = find700(parseHex('#1A6646'), 'white')
    const shade100 = find100(shade700, 'white')
    const shade600 = find600(shade100, 'white')
    expect(contrastRatioHex(find800(shade600, 'white'), shade600)).toBeGreaterThanOrEqual(3.1)
  })

  it('black theme: result has >= 3.1 contrast against shade600', () => {
    const shade700 = find700(parseHex('#1C734E'), 'black')
    const shade100 = find100(shade700, 'black')
    const shade600 = find600(shade100, 'black')
    expect(contrastRatioHex(find800(shade600, 'black'), shade600)).toBeGreaterThanOrEqual(3.1)
  })
})
...
describe('find900', () => {
  it('white theme: result has >= 3.1 contrast against shade700', () => {
    const shade700 = find700(parseHex('#1A6646'), 'white')
    expect(contrastRatioHex(find900(shade700, 'white'), shade700)).toBeGreaterThanOrEqual(3.1)
  })

  it('white theme: result is darker than shade700', () => {
    const shade700 = find700(parseHex('#1A6646'), 'white')
    const shade900 = find900(shade700, 'white')
  ...
  it('black theme: result has >= 3.1 contrast against shade700', () => {
    const shade700 = find700(parseHex('#1C734E'), 'black')
    expect(contrastRatioHex(find900(shade700, 'black'), shade700)).toBeGreaterThanOrEqual(3.1)
  })

  it('black theme: result is lighter than shade700', () => {
    const shade700 = find700(parseHex('#1C734E'), 'black')
    const shade900 = find900(shade700, 'black')
